...
function getTopicContent(
  ...
  const prefix = `Topics.items.${service.key}` as const;
...
export async function generateMetadata({
  ...
  const content = getTopicContent(t, service);
...
export default async function ServicePage({
  ...
  const t = await getTranslations({ locale });
  const content = getTopicContent(t, service);
  const relatedTopics = serviceDefinitions.filter((item) => item.slug !== service.slug);
      ...
      <div className="mx-auto max-w-7xl space-y-14 sm:space-y-20 2xl:max-w-[88rem]">
        <div className="flex justify-start">
          ...
          >
            ...
            <span>{t("Topics.common.backToTopics")}</span>
          </Link>
        </div>
        ...
        <section className="space-y-6">
          <div className="text-left">
            <p className="text-sm font-medium tracking-wide text-[var(--color-accent-light)]">
              {t("Topics.common.relatedTopics")}
            </p>
          </div>
          <div className="grid gap-5 md:grid-cols-2">
            {relatedTopics.map((relatedService) => {
              const relatedContent = getTopicContent(t, relatedService);
